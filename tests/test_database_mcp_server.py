...
def test_run_select_allowed():
    result = run_select("SELECT * FROM customers LIMIT 2")
    assert "rows" in result

def test_run_select_blocks_insert():
    result = run_select("INSERT INTO customers VALUES (99,'x','x@x.com','US','now')")
    assert "error" in result

def test_run_select_blocks_drop():
    result = run_select("DROP TABLE customers")
    assert "error" in result
